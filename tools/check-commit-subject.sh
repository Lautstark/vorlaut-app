...
Android app, a plain-JVM parser and a release pipeline. Put it in front of the
...
A scope is optional and is a path-ish word, not a sentence: fix(boardpackage):,
docs(releasing):.
...
says what changed, in the present tense. The prefix goes in front of it:
  ...
  feat: render a board package without unpacking it to disk first
  fix: refuse an .obz whose manifest names a file it does not carry
